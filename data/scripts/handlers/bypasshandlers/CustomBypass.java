...
public class CustomBypass implements IBypassHandler
{
	...
	public boolean useBypass(String command, L2PcInstance player, L2Npc target)
	{
		...
		else if (command.startsWith("changerace"))
		{
			...
			if (!player
					.destroyItemByItemId("SpecialServices", Config.DONATION_COIN_ID, Config.CHANGE_RACE_PRICE, player,
							true))
			{
				player.sendMessage("Special Services: You don't have enough coins!");
				return false;
		...
		else if (command.startsWith("changeclanname") || command.startsWith("changecharname"))
		{
			...
			if (command.equalsIgnoreCase("changeclanname") || command.equalsIgnoreCase("changecharname"))
			{
				...
				sb.append("Tired of your current " + changeWhat +
		...
		else if (command.equalsIgnoreCase("increaseclanlevel"))
		{
			...
			if (playerClan == null || !player.isClanLeader() || (playerClan.getLevel() == 10))
			{
			...
			if (!player
					.destroyItemByItemId("SpecialServices", Config.DONATION_COIN_ID, Config.INCREASE_CLAN_LEVEL_PRICE,
							target, true))
			{
				return false;
			}
